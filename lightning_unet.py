...
class LitUNet(pl.LightningModule):
    ...
    def __init__(self, file_pairs, input_num=4, output_num=1, initial_feat=32, trained=False):
        ...
        if not ENCODER:
            self.model = smp.Unet(classes=OUTPUT_CHANNELS, in_channels=INPUT_CHANNELS, aux_params=aux, encoder_depth=7,
                                  decoder_channels=decode_ch)
        else:
            self.model = smp.Unet(ENCODER, classes=OUTPUT_CHANNELS, in_channels=INPUT_CHANNELS, aux_params=aux,
                                  encoder_depth=7, decoder_channels=decode_ch)
...
if __name__ == "__main__":
    ...
    if REP == 16:
        trainer = pl.Trainer(gpus=gpus, max_epochs=MAX_EPOCHS, logger=nep, profiler=True, precision=16, auto_lr_finder=True)
    else:
        trainer = pl.Trainer(gpus=gpus, max_epochs=MAX_EPOCHS, profiler=True, logger=nep, auto_lr_finder=True)
    start = time.time()
    trainer.fit(model)
    end = time.time()
