...
if __name__ == "__main__":
    ...
    if REP == 16:
        trainer = pl.Trainer(gpus=gpus, max_epochs=MAX_EPOCHS, logger=nep, early_stop_callback=True, precision=16)
    else:
        trainer = pl.Trainer(gpus=gpus, max_epochs=MAX_EPOCHS, logger=nep, early_stop_callback=True)
    start = time.time()
    trainer.fit(model)
    end = time.time()
